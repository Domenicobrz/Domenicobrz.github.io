...
import { mat4, vec3 } from "./gl-matrix-min";

var zoom = 45;	// la camera così com'è è broken perchè non è fatta per supportare gli import di webpack per cui lo zoom è broken
...
function camera_getAutocenteredViewMatrix(deltatime, smoothness, radius)
{
    ...
    // if(this.pitch > 3.13/2)  this.pitch = 3.13/2;
    // if(this.pitch < -3.13/2) this.pitch = -3.13/2;
    if(this.pitch > 0.65)  this.pitch = 0.65;
    if(this.pitch < 0.08) this.pitch = 0.08;
