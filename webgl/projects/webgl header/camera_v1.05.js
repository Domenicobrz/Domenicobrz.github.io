...
function createCamera()
{
	...
	if(this.mousecontrols)	{
		...
		document.addEventListener("mousemove", this.camera_mousemove.bind(this));
		...
		document.addEventListener("touchmove", this.camera_touchmove.bind(this));
...
var camera_scr_yaw   = -3.14 / 2;
var camera_scr_pitch = 0;
function camera_mousemove(e)
{
	if(this.non_click_movement) {
		var maxYaw   = 3.14/4;
		var maxPitch = 3.14/4;

		camera_scr_yaw   = e.clientX / window.innerWidth * maxYaw - maxYaw / 2.0 - 3.14 / 2.0;
		camera_scr_pitch = e.clientY / window.innerHeight * maxPitch - maxPitch / 2.0;
		return;
	}
...
{
	if(e.which != 1) return;
	if(this.non_click_movement) return;
	this.lastPos[0] = e.clientX;
	this.lastPos[1] = e.clientY;
	this.dragging = true;
}
...
function camera_getAutocenteredViewMatrix(deltatime, smoothness, radius)
{
    ...
    if(this.non_click_movement) {
    	var distanceFromYaw   = (camera_scr_yaw   - this.yaw)   * 0.01;
		var distanceFromPitch = (camera_scr_pitch - this.pitch) * 0.01;

		this.yaw   += distanceFromYaw;	
		this.pitch += distanceFromPitch;
    }
    ...
    if(this.depth_of_field_transform_count != 0) {
    	...
    	var degree = 6.28 * this.depth_of_field_transform_count / 8;

    	var yaw   = this.yaw   + Math.cos(degree) / 115; //150;
    	var pitch = this.pitch + Math.sin(degree) / 115; //150;
...
//v1.06		-	non-click camera movement (needs autocentered to make any sense)