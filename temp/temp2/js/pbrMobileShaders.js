function mobilePBRmaterial(params) {
    ...
    {
        return F0 + (max(vec3(1.0 - roughness), F0) - F0) * pow(1.0 - cosTheta, 10.0);  // era 5.0
    } 
    ...
    vec3 getPrefilteredColor(vec3 R, float roughness) {
        ...
        const float switchOffset = 0.4;

        if (roughness < switchOffset) {
            return textureCube(spec1, R, roughness * MAX_REFLECTION_LOD).rgb;
        } else {
            return textureCube(spec2, R, (roughness - switchOffset) * MAX_REFLECTION_LOD).rgb;
        }
    }
    ...
    void main() {
        ...
        gl_FragColor = vec4(   textureCubeLodEXT(spec1, R, roughness * 10.0).rgb    , 1.0);
    }`;
